...
class BuildAutomation:
    ...
    def get_latest_commit(self):
        ...
        if result is None or result.returncode != 0:
            logger.error(
                f"Git fetch failed: {result.stderr if result else 'Timeout'}")
            return None
    ...
    def pull_latest(self):
        ...
        if result and result.returncode == 0:
            ...
            logger.error(
                f"Pull failed: {result.stderr if result else 'Unknown error'}")
            return False
    ...
    def backup_build(self):
        ...
        # Clean old backups
        backups = sorted(backup_dir.glob("build_*.apk"),
    ...
    def upload_to_website(self):
        ...
        try:
            ...
            with open(self.apk_path, "rb") as f:
                files = {"file": (self.apk_path.name, f,
    ...
    def upload_to_github_release(self, commit_hash, commit_message):
        ...
        try:
            ...
            response = requests.post(
                release_url, headers=headers, json=release_data)

            if response.status_code == 201:
                release_info = response.json()
                upload_url = release_info["upload_url"].replace(
                    "{?name,label}", "")
                ...
                if response.status_code == 200:
                    ...
                    logger.error(
                        f"Failed to create/get release: {response.text}")
                    return False
            else:
                logger.error(
            ...
            if upload_response.status_code == 201:
                ...
                logger.error(
                    f"Asset upload failed: {upload_response.status_code} - {upload_response.text}")
                return False
    ...
    def send_notification(self, success, message=""):
        ...
        try:
            ...
            requests.post(NOTIFICATIONS["webhook_url"],
    ...
    def run_full_pipeline(self):
        ...
        commit_info = self.get_commit_info(
            self.last_commit_hash) if self.last_commit_hash else ""
